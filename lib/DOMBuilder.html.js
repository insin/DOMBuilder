 ...
 * @param {string|ak.template.Wrap} value
 * @return {boolean}
 */
...
function escapeHTML(s) {
  return ak.escapeHTML(s);
}

/**
 * If the given input is safe, returns its value; otherwise, coerces
...
MockElement.prototype.toString = function(trackEvents) {
  ...
  if (hasOwn.call(this.attributes, 'innerHTML')) {
    ...
    for (var i = 0, l = this.childNodes.length, node; i < l; i++) {
      node = this.childNodes[i];
      if (node instanceof MockElement || node instanceof ak.template.Wrap) {
        parts.push(node.toString(trackEvents));
      }
      else {
        // Coerce to string and escape
        parts.push(ak.escapeHTML(''+node));
      }
    }
  }
...
MockElement.prototype.toString.safe = true;
...
MockFragment.prototype.toString.safe = true;
...
, isModeObject: function(obj) {
    return (obj instanceof HTMLNode ||
            obj instanceof ak.template.Wrap);
  }
, api: {
