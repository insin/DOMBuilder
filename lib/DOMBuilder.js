...
/**
 * Rules out false positives when trying to determine whether or not an
 * attributes Object was passed. This depends on the mode we're rendering in,
 * which is given.
 */
function isPlainObject(o, mode) {
  return (!!o &&
          toString.call(o) == '[object Object]' &&
          (!mode || DOMBuilder.modes[mode].isPlainObject(o)));
}

/**
 * Returns ``false`` for Arrays which represent elements.
 */
...
/**
 * Indicates that an array represents an element.
 */
...
function createElementFunctions(mode) {
  ...
  for (var i = 0, tag; tag = TAG_NAMES[i]; i++) {
    obj[tag] = obj[tag.toUpperCase()] = createElementFunction(tag, mode);
  }
  return obj;
}
...
, modes: {}
   ...
   *    should return ``false`` if given a mode-specific content object.
   * ``api`` (optional)
   *    an object defining additional public API for the mode and exposing the
   *    variables, functions and constructors used in its implementation, if
   *    appropriate.
   */
, addMode: function(mode) {
    mode = extend({isPlainObject: function() { return true; }, api: {}}, mode);
    ...
    // API as a DOMBuilder.<modename> property.
...
, createElement: function(tagName, attributes, children, mode) {
    ...
    if (mode) {
      ...
      return elementArray([tagName, attributes].concat(children));
    }
  }
