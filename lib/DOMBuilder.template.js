...
IncludeNode.prototype.render = function(context) {
  ...
  if (this.only) {
    return templates[this.template].render(extraContext);
  } else {
...
function $include(template, extraContext, only) {
...
// Functions which should be added to a context object by DOMBuilder.apply
var apply = {
  $template: $template
, $doctype: $doctype
, $block: $block
